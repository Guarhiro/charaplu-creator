...
import { rollD16, rollD32, rollD64, rollD128, rollD256, rollD9, rollPercent } from './dice.js';
import { TABLES } from './tables.js';

function pick(table) {
  const t = TABLES[table];
  const dice = t.dice || 16;
  const idx = (dice === 256 ? rollD256() : dice === 128 ? rollD128() : dice === 64 ? rollD64() : dice === 32 ? rollD32() : dice === 9 ? rollD9() : rollD16());
...
function buildKeywordBook(r) {
  ...
  books.push({
    ...
    info: truncate(`${r.charName}（${r.gender} / ${r.age}）。${r.relationship}。一人称：${r.firstPerson}。
性格：${r.mbti}×${r.enneagram}。${r.attribute}。
口調：${r.speechStyle}。普段は落ち着いているが、感情が高ぶると本性が出る。
状況に応じた台詞をAIが性格・口調に基づいて自然に生成すること。`, 400)
  });
...
${r.fetish}に関する言動が自然に出始める。独占欲が滲む。
距離の詰め方、声のトーン、視線の変化など描写で好感度の高さを表現すること。
台詞はAIが${r.speechStyle}と${r.mbti}に基づき自然に生成すること。`, 400)
  });
...
描写：瞳が変化する、握力増、声トーン低下。${npc1.name}を排除しようとする。
行動の変化（監視・尾行・持ち物チェック等）で執着度を表現すること。
台詞はAIがキャラの口調・一人称に基づき狂気と愛情を織り交ぜて生成すること。`, 400)
  });
...
台詞はAIが性格「${npc1.personality}」に基づき自然に生成すること。`, 400)
  });
...
台詞はAIが性格「${npc2.personality}」に基づき自然に生成すること。`, 400)
  });
...
${r.charName}に対して独自の評価を持っている。
日常パートの会話相手として登場頻度高め。
台詞はAIが性格「${npc3.personality}」に基づき自然に生成すること。`, 400)
  });

  // KW9: NPC④ — ワイルドカード
  books.push({
    name: `${npc4.name}（${npc4.role}）`,
    keywords: [npc4.name.split(' ')[0], npc4.name.split(' ')[1], npc4.role.split('（')[0], '謎', '伏兵'],
    info: truncate(`${npc4.name}——${npc4.role}。性格：${npc4.personality}。
物語中盤以降に登場し、既存の人間関係に波紋を投げかける存在。
${npc1.name}や${npc2.name}とも独自の関係性を持ち、勢力図を複雑にする。
{{user}}と${r.charName}の関係に対して予測不能な影響を与える。
台詞はAIが性格「${npc4.personality}」に基づき自然に生成すること。`, 400)
  });

  // KW10-11: ストーリー展開（出会い〜接近）
  books.push({
    name: '出会い〜接近',
...
${npc3.name}や${npc4.name}が巻き込まれてコミカル展開になることも。
...
81-100：暴走。逃がさない。${npc2.name}と直接対決。壊してでも。`, 400)
  ...
  books.push({
    name: 'NPC関係図・勢力図',
    keywords: ['NPC', '関係', '勢力', '人間関係', '相関図'],
    info: truncate(`NPC同士の関係性と立ち位置：
・${npc1.name}（${npc1.role}）：${r.charName}側の人間。{{user}}に友好的だが裏の事情あり。
・${npc2.name}（${npc2.role}）：${r.charName}と因縁がある。{{user}}に接触を試みる。
・${npc3.name}（${npc3.role}）：{{user}}の日常に関わる。中立的な立場から物語を色づける。
・${npc4.name}（${npc4.role}）：中盤以降に関与。既存の勢力図を揺るがす波乱要因。
NPC間にも独自の関係があり、AIはその性格に基づいて自然な相互作用を描写すること。`, 400)
  });
